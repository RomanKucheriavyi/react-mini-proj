import React, { useState, useEffect } from "react";
import VideoBg from "./components/VideoBg";
import Trivia from "./components/Trivia";
import { useDataFetching } from "../../helper/game/Data-fetcher";
import { Link } from "react-router-dom";
import "./Game.css"

const Game = () => {
    const [questionNumber, setQuestionNumber] = useState(11);
    const {data, isLoading} = useDataFetching();
    const [isOver, setIsOver] = useState(false);
    const [earned, setEarned] = useState(0);
    const [isLose, setIsLose] = useState(false);

    const moneyList = 
        [
            { id: 1, amount: "100" },
            { id: 2, amount: "200" },
            { id: 3, amount: "300" },
            { id: 4, amount: "500" },
            { id: 5, amount: "1.000" },
            { id: 6, amount: "2.000" },
            { id: 7, amount: "4.000" },
            { id: 8, amount: "8.000" },
            { id: 9, amount: "16.000" },
            { id: 10, amount: "32.000" },
            { id: 11, amount: "64.000" },
            { id: 12, amount: "125.000" },
            { id: 13, amount: "250.000" },
            { id: 14, amount: "500.000" },
            { id: 15, amount: "1.000.000" },
        ].reverse();

    useEffect(() => {
        if (questionNumber > 1) {
            setEarned(moneyList.find((m) => m.id === questionNumber - 1).amount);
        }
    }, [questionNumber, moneyList]);

    return (
        <div className="game">
            <div className="main">
                <VideoBg />
                <Trivia 
                    data = {data} 
                    setQuestionNumber={setQuestionNumber}
                    questionNumber = {questionNumber}
                    isLoading = {isLoading}
                    isLose = {isLose}
                    isOver = {isOver}
                    setIsLose = {setIsLose}
                    earned = {earned}
                />
            </div>

            <div className="info-panel">
                <ul className="money-list">
                {moneyList.map((money) => (
                    <li
                    className={
                        questionNumber === money.id
                        ? "money-list__item_active"
                        : "money-list__item"
                    }
                    key={money.id}
                    >
                    <span className="money-list__number">{money.id}</span>
                    <span className="money-list__amount">{money.amount}</span>
                    </li>
                ))}
                </ul>
                <div className="button-group">
                    <button><Link to="/">Home</Link></button>
                    <button onClick={() => (setIsOver(true))}>End Game</button>
                </div>
            </div>
        </div>
    );
};

export default Game;